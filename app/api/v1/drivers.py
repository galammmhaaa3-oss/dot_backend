...
async def upload_document(
    ...
    """Upload a driver document (photo)."""
    # Validate file type
    ...
    # Return relative URL
    file_url = f"/uploads/drivers/{current_user.id}/{unique_filename}"
...
def register_driver(
    ...
    if existing_driver:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="You already have a driver application"
        )
    
    # Check if national ID is already used
    existing_national_id = db.query(Driver).filter(Driver.national_id == driver_data.national_id).first()
